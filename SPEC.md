...
| static region | AXI/control wrapper, checker/mode FSM, Schmitt mode arbiter (`checker_hysteresis.v`, 2026-08-06), reconfiguration control = **AMD DFX Controller IP (PG374, adopted 2026-08-06** via `dfxc_trigger_adapter.v` + `pr_latency_probe.v` for latency measurement; custom `pr_controller.v` archived), output/metadata packer (**the baseline ISP core is NOT static** — see the RP-boundary row below) |
